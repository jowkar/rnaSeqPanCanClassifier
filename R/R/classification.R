...
.cv.eval.k <- function(r.tcga, refset.custom, params, k){
  ...
  for (cn in colnames(res.class.tcga)){
    ...
    # Get rid of warning about factor level mismatches

    missing_1 <- setdiff(union(tmp_1,tmp_2),tmp_1)
    missing_2 <- setdiff(union(tmp_1,tmp_2),tmp_2)
    tmp_1f <- factor(tmp_1,levels=sort(c(unique(tmp_1),missing_1)))
    tmp_2f <- factor(tmp_2,levels=sort(c(unique(tmp_2),missing_2)))
    stopifnot(identical(as.character(tmp_1f),as.character(tmp_1)))
    stopifnot(identical(as.character(tmp_2f),as.character(tmp_2)))
    stopifnot(identical(levels(tmp_1f),levels(tmp_2f)))
    tmp_1 <- tmp_1f
    tmp_2 <- tmp_2f
    rm(tmp_1f)
    rm(tmp_2f)

    cv.tab[[cn]] <- caret::confusionMatrix(tmp_1,tmp_2)
  }
...
#' @export
cv <- function(annot=NULL, params=NULL, refset, max_k=20, nthreads = 4){
  ...
  if (!is.null(nthreads) && nthreads > 1){
    ...
    cv.res.k <- foreach (k = 1:max_k, .packages = c("Biobase")) %dopar% {
      attach(environment(classification:::.cv.eval.k))
      attach(environment(classification:::.knn.cv))
      attach(environment(classification:::.calculate_correlations))
      tmp <- .cv.eval.k(r.tcga, refset.custom, params, k=k)
      tmp$k <- k
      tmp
    }
    ...
    stopifnot(identical(unlist(lapply(cv.res.k,function(x){x$k})),1:max_k))
  } else {
    cv.res.k <- list()
    for (k in 1:max_k){
